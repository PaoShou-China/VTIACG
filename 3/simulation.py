...
class Missile:
    ...
    def run(self, target_pos, target_vel):
        ...
        acc_desired = self.N * bearing_rate * np.linalg.norm(self.vel)
        self.acc += (acc_desired - self.acc) / T * dt

        acc_x = self.acc * self.vel[1] / np.linalg.norm(self.vel)
        acc_y = -self.acc * self.vel[0] / np.linalg.norm(self.vel)

        self.vel += np.array([acc_x, acc_y]) * dt
        self.pos += self.vel * dt
        Theta = np.arctan2(self.vel[1], self.vel[0])
        return np.copy(self.pos), np.copy(self.vel), -acc_desired + g * np.cos(Theta)
...
def guidance_data(parameter, guidance_law):
    ...
    while t < 300:
        ...
        selfacc += (a - selfacc) / T * dt
        if np.linalg.norm(a) > 50:
            selfacc = selfacc / np.linalg.norm(a) * 50

        dthetadt = selfacc / V - g * np.cos(theta) / V
